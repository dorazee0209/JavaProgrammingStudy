...
## 미리 정의된 함수형 인터페이스 4형제

`java.util.function` 패키지에는 자주 쓰이는 형태의 함수형 인터페이스가 미리 정의되어 있다. 그중 핵심이 되는 네 가지는 다음과 같다.

| 인터페이스 | 추상 메소드 | 매개변수 | 반환 | 한 줄 요약 |
|---|---|---|---|---|
| `Predicate<T>` | `boolean test(T t)` | 있음 (1개) | `boolean` | 판단한다 — 조건에 맞나? |
| `Supplier<T>` | `T get()` | 없음 | `T` | 공급한다 — 만들어서 준다 |
| `Consumer<T>` | `void accept(T t)` | 있음 (1개) | `void` | 소비한다 — 받아서 처리만 |
| `Function<T, R>` | `R apply(T t)` | 있음 (1개) | `R` | 변환한다 — T를 받아 R로 |

매개변수 유무와 반환값 유무의 네 가지 조합을 그대로 인터페이스로 만들어 둔 것으로 보면 된다. 이 네 가지를 알면 나머지는 모두 이들의 변형이다.
