...
- 그런데 **캐스트 표현식은 그 자체로 단독 문장(statement)이 될 수 없다**는 규칙이 Java에 있다. (문장으로 허용되는 건 대입문, `++`/`--`, 메소드 호출, 객체 생성 등)
...
PaperBox pb = (PaperBox) box.makeSomething();   // ✅ 리턴값(Object)을 PaperBox로 캐스팅해 대입 — 정상 동작
```
...
→ 어떤 타입을 넣어도 가장 바깥쪽 조건이 항상 먼저 걸려서 `Simple Wrapping`만 세 번 출력된다.
